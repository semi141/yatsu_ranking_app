...
#### ランキング画面（全体ランキング）
![ランキング画面のスクリーンショット](https://raw.githubusercontent.com/semi141/yatsu_ranking_app/main/images/新ランキングページ.PNG)
...
#### マイページ・認証
* **認証機能:** Google OAuthによる認証と、一時試用向けのゲストログインに対応。（※ゲストデータは24時間後に自動削除）
...
* **日本語メッセージ:** ログイン/ログアウト時のFlashメッセージを日本語化し、ユーザー体験を向上。
* **ヘッダーデザイン:** アクションボタンとナビゲーションボタンで配色にメリハリをつけ、**未ログイン時にも「ゲストログイン」と「Googleログイン」が並列で分かりやすく表示される**よう視認性を最適化。
...
* **API連携とセキュリティ強化:**
    * 外部のChrome拡張機能からのデータ受信に特化した**厳格なCORS設定**を行い、**セキュリティを担保**しました。
    * 視聴回数の自動更新を担う動画プレイヤーには、**カウント重複防止ロジック**を実装し、データの正確性を保証しています。
* **ゲストユーザー機能と堅牢性:**
    * **ゲストユーザーの自動削除と非同期処理: Heroku Schedulerアドオンを用いてrakeタスクを毎日自動実行することで、ゲストデータを非同期かつ確実に削除する運用を実装し、システムの負荷とセキュリティリスクを軽減しています。
* **データベース設計と効率性:**
    * 視聴記録（Watch）を独立させ、ActiveRecordの集計メソッドを用いて期間別ランキングを効率的に計算しています。
* **テストと品質保証:**
    * Minitestを使用し、**Model Spec**（視聴データ集計、ゲストユーザーの自動削除ロジック）や**System Spec**を実装することで、システムの信頼性を保証しました。
* **UI/UXの改善:**
    * ログイン/ログアウトなどの機能に応じた**ヘッダーUIの配色**、そして誤操作を防ぐ独自の**タグ編集モード**を実装し、ユーザー体験を向上させています。
