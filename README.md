...
```text
User
 ├── api_token : string (Chrome拡張機能の認証用)
 └── has_many : watches, posts
     
Video
 ├── title : string
 ├── youtube_id : string (UNQ)
 └── has_many : watches, posts, taggings

Watch (視聴記録)
 ├── user_id : integer
 ├── video_id : integer
 └── watched_count : integer (視聴回数)
